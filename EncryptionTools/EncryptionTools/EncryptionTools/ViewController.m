...
- (void)hashTest {
    NSString *plainStr = @"P8GmSsx5wlFAKq7ilMqfZQ==pCEC9AbzFaOHF/XjPGwcyQ==";
    NSString *key = @"ig+5V8EuJAUwUNtFhAyw6UBvj7V+PC//+C8MGRrMWXs=";//"ig+5V8EuJAUwUNtFhAyw6UBvj7V+PC//+C8MGRrMWXs=";
    
//    NSData *keyData = [key dataUsingEncoding:NSUTF8StringEncoding];
//    NSString *keys = [keyData base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
    
    NSData *keyData = [[NSData alloc] initWithBase64EncodedString:key options:NSDataBase64DecodingIgnoreUnknownCharacters];
//    NSString *keys = [[NSString alloc] initWithData:keyData encoding:NSASCIIStringEncoding];
    ...
    NSLog(@"hmacSHA1:%lu---%@---%@",[plainStr hmacSHA1WithKey:key].length,[plainStr hmacSHA1WithKey:key],hmacSHA1.hex);
    
    CocoaSecurityResult *hmacSHA256 = [CocoaSecurity hmacSha256:plainStr hmacKey:keyData];
    NSLog(@"hmacSHA256:%lu---%@---%@",[plainStr hmacSHA256WithKey:keyData].length,[plainStr hmacSHA256WithKey:keyData],hmacSHA256.hex);
    
    //NSString *sValue = [CocoaSecurity hmac256:plainStr withKey:keys];
    //NSLog(@"sValue:%@",sValue);
}
